...
describe('resolverSerie', () => {
  it('un set vacío hereda todo del ejercicio', () => {
    expect(resolverSerie(ej, {})).toEqual({
      reps: '12-15', rest_seconds: 90, tempo: '', rir: '2', rpe: '', pct_1rm: '', ref_weight: 20, set_type: 'efectiva',
    });
  });
...
describe('aplanarSeries', () => {
  it('el ejercicio lleva el set 1 y los sets solo lo distinto', () => {
    ...
    expect(ejercicio).toEqual({
      reps_objective: '12-15', rest_seconds: 90, tempo: null, target_rir: '2', target_rpe: null, target_pct_1rm: null, ref_weight: 20,
    });
    expect(series[0]).toEqual({
      reps_objective: null, rest_seconds: null, tempo: null, target_rir: null, target_rpe: null, target_pct_1rm: null, ref_weight: null, set_type: null,
    });
    expect(series[2]).toEqual({
      reps_objective: '8', rest_seconds: null, tempo: null, target_rir: null, target_rpe: null, target_pct_1rm: null, ref_weight: 25, set_type: 'fallo',
    });
  });
...
describe('lineaSerie', () => {
  const s = { reps: '12-15', rest_seconds: 90, tempo: '', rir: '2', rpe: '', pct_1rm: '80/85%', ref_weight: 60, set_type: 'efectiva' as const };

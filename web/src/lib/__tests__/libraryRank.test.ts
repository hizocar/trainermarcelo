...
import { rankLibrary, normalizar } from '../libraryRank';

// Mismos casos que trainer-app/src/lib/__tests__/bibliotecaRank.test.ts:
...
describe('rankLibrary', () => {
  ...
  it('a igual puntaje, el nombre corto (el básico) antes que el rebuscado', () => {
    const r = rankLibrary([lib('Sentadilla búlgara con salto'), lib('Sentadilla')], 'sentadilla');
    expect(r[0].name).toBe('Sentadilla');
  });

  it('busca sin tildes y en el nombre en inglés', () => {
    expect(normalizar('Prés')).toBe('pres');
    const r = rankLibrary([lib('Peso muerto rumano', null, 'Romanian deadlift')], 'romanian');
    expect(r).toHaveLength(1);
  });

  it('lo que no coincide, fuera', () => {
    expect(rankLibrary([lib('Curl bíceps')], 'sentadilla')).toHaveLength(0);
  });
});
